...
## 3. Удаление аккаунта — реализовано

Требование Apple (App Review Guideline 5.1.1(v)) и Google Play (Data deletion): при наличии регистрации нужен способ удалить аккаунт и данные из приложения.

Сделано:
- бэкенд: `DELETE /v1/me` (`server/src/modules/auth/routes.ts`) — в транзакции удаляет `NotificationLog` → `AlertRule` → `DeviceToken` → `AppUser` (FK-каскада в схеме нет, удаляем детей явно); refresh-токены stateless, после удаления `/v1/auth/refresh` вернёт 401;
- клиент: экран «Ещё» → «Удалить аккаунт» с подтверждением (`mobile/app/(tabs)/more.tsx`), после успеха — разлогин.

Остаётся для Google: указать в консоли **публичный URL для запроса удаления** — можно ту же страницу политики конфиденциальности с описанием пути «Ещё → Удалить аккаунт» и почтой поддержки.
...
- [x] Реализовать удаление аккаунта (§3) — сделано. Для Google указать публичный URL запроса удаления.
