...
async function marketauxQuery(
  params: Record<string, string>,
): Promise<MarketauxResponse> {
  const query = new URLSearchParams({
    api_token: MARKETAUXAPI_KEY ?? "",
    ...params,
  }).toString();
...
function printSummary(label: string, res: MarketauxResponse) {
  ...
  if (res.warnings?.length)
    console.log(`  Предупреждения: ${res.warnings.join("; ")}`);
  if (!res.data?.length) {
  ...
  for (const a of res.data) {
    ...
    const countries =
      [...new Set(a.entities.map((e) => e.country))].join(",") || "—";
    const symbols =
      a.entities
        .map((e) => e.symbol)
        .slice(0, 5)
    ...
    console.log(
      `    types=${types}  countries=${countries}  symbols=${symbols}`,
    );
  }
}
...
async function main() {
  if (!MARKETAUXAPI_KEY) {
    console.error(
      "MARKETAUXAPI_KEY не задан в server/.env — нечего проверять.",
    );
    process.exit(1);
  }
  ...
  const checks: Array<{ label: string; params: Record<string, string> }> = [
    {
      label: "entity_types=currency — размечаются ли валютные сущности?",
      params: { entity_types: "currency" },
    },
    {
      label: "language=ru — есть живые русскоязычные источники?",
      params: { language: "ru" },
    },
    {
      label: "countries=kz — есть казахстанские источники?",
      params: { countries: "kz" },
    },
    {
      label: "search=tenge — что находится про KZT глобально?",
      params: { search: "tenge" },
    },
  ];
