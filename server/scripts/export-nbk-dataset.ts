...
async function main() {
  ...
  console.log(`Собираю дневной датасет НБ РК за ${toIso(start)} … ${toIso(end)} (${RANGE_DAYS} дней)`)
  ...
  while (cursor <= end) {
    ...
    try {
      const xml = await fetchWithRetry(`https://nationalbank.kz/rss/get_rates.cfm?fdate=${toRuDate(cursor)}`)
