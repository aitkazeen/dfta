...
const ALLOWED_CATEGORIES = new Set<DriverCategory>(['technical', 'news', 'regulator', 'global'])
const MAX_DRIVERS = 3
...
export function buildUserPrompt(input: ExplainInput): string {
  const indicatorLines = Object.entries(input.indicators)
    ...
    .map(([name, value]) => `${INDICATOR_LABELS[name] ?? name}: ${value!.toFixed(4)}`)
    .join('\n')
  ...
  const drivers = obj.drivers
    .filter(
      (d): d is { category: unknown; text: unknown } =>
        typeof d === 'object' && d !== null && typeof (d as Record<string, unknown>).text === 'string',
    )
    .slice(0, MAX_DRIVERS)
    .map((d) => ({
      ...
      category: ALLOWED_CATEGORIES.has(d.category as DriverCategory) ? (d.category as DriverCategory) : 'technical',
