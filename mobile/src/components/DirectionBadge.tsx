...
const SIZES: Record<Size, { padV: number; padH: number; radius: number; fontSize: number; weight: string }> = {
  xs: { padV: 2, padH: 7, radius: 6, fontSize: 11, weight: fontFamily.semibold },
  sm: { padV: 3, padH: 8, radius: radius.sm, fontSize: 13, weight: fontFamily.medium },
  md: { padV: 4, padH: 10, radius: radius.sm, fontSize: 13, weight: fontFamily.semibold },
}
...
export function DirectionBadge({ direction, label, size = 'sm' }: Props) {
  ...
  return (
    ...
    >
      <Text
        tabular
        style={{ color: palette.fg, fontFamily: cfg.weight, fontSize: cfg.fontSize, lineHeight: cfg.fontSize + 5 }}
      >
