 ...
 */
export function mapIndicators(raw: ApiIndicators, rate: number, symbol: string): Indicator[] {
    const rows: Indicator[] = []

    const rsi = Number(raw.rsi14?.value)
    if (rsi != null) {
        const tone: IndicatorTone = rsi >= 70 ? 'warn' : rsi <= 30 ? 'warn' : 'neutral'
        const interp = rsi >= 70 ? 'Перекуплен' : rsi <= 30 ? 'Перепродан' : 'Нейтрально'
    ...
    if (macd != null) {
        ...
        const interp = macd > 0 ? 'Восходящий импульс' : macd < 0 ? 'Нисходящий импульс' : 'Нейтрально'
        rows.push({ name: 'MACD', value: `${macd > 0 ? '+' : ''}${macd.toFixed(2)}`, interp, tone })
    }
    ...
    if (atr != null) {
        ...
        const interp = pctOfRate > 1 ? 'Повышенная волатильность' : 'Обычная волатильность'
        rows.push({ name: 'ATR (14)', value: `${atr.toFixed(2)} ${symbol}`, interp, tone })
    }
