...
export default function FullForecastScreen() {
  ...
  useEffect(() => {
    ...
    Promise.all([getQuote(id), getForecast(id).catch(() => null)])
      ...
      .catch((err) =>
        console.error("[forecast] не удалось загрузить курс/прогноз", err),
      );
      ...
      >
        ...
        <View style={styles.blockH}>
          <Card>
            ...
            >
              Точность рассчитывается по совпадению направления движения, а не
              точной цены. Публикуем всегда, даже когда результат ниже среднего.
            </Text>
          </Card>
        </View>
